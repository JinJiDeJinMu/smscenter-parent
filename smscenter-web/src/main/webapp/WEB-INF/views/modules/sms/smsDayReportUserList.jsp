...
<head>
	...
	<script type="text/javascript">
		...
		function page(n,s){
			if(n) $("#pageNo").val(n);
			if(s) $("#pageSize").val(s);
			$("#searchForm").attr("action","${ctx}/sms/smsDayReport/listByUser?queryType=day");
			$("#searchForm").submit();
        	return false;
        }	
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/sms/smsDayReport/listByUser?queryType=day">信息列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="smsDayReport" action="${ctx}/sms/smsDayReport/listByUser?queryType=day" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="smsDayReport">
			<tr>
				...
				<td>
					${smsDayReport.pushSuccessCount}
				</td>
				<td>
					<fmt:formatNumber type="number" value="${smsDayReport.pushSuccessCount*100/smsDayReport.sendCount}" maxFractionDigits="2" pattern="0.00"></fmt:formatNumber>%
				</td>
			</tr>
		</c:forEach>
		</tbody>
