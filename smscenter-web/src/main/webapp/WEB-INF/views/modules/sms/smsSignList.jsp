...
<head>
	...
	<script type="text/javascript">
		...
		function sigleDelete(id, userId, gwCode, sign)
		{
			sign = encodeURI(encodeURI(sign));
			top.$.jBox.confirm("确认要删除该通道签名吗？","系统提示",function(v,h,f){
				if(v=="ok"){
					$("#searchForm").attr("action","${ctx}/sms/smsSign/delete?id="+id+"&userId="+userId+"&tmpSign="+sign+"&tmpGatewayId="+gwCode);
					$("#searchForm").submit();
				}
			},{buttonsFocus:1});
			top.$('.jbox-body .jbox-icon').css('top','55px');
		...
		function showTestModal(gwCode, spNumber,sign,userId){
			$('.modal-body').find('input[id="gatewayId"]').val(gwCode);
...
<body>
	<div id="deleteDiv" class="hide">
		<form id="deleteForm" action="${ctx}/sms/smsSign/batchDelete" method="post">
			<input type="hidden" id="ids" name="ids">
		</form>
	</div>
	<div id="importBox" class="hide">
		<form id="importForm" action="${ctx}/sms/smsSign/import" method="post" enctype="multipart/form-data"
			class="form-search" style="padding-left:20px;text-align:center;" onsubmit="loading('正在导入，请稍等...');"><br/>
			<input id="uploadFile" name="file" type="file" style="width:330px"/><br/><br/>　
			<input id="btnImportSubmit" class="btn btn-primary" type="submit" value="   导    入   "/>
			&nbsp;&nbsp;
			<a href="${ctx}/sms/smsSign/import/template">下载模板</a>
		</form>
	</div>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/sms/smsSign/">信息列表</a></li>
		<shiro:hasPermission name="sms:smsSign:edit">
			<%-- <li><a href="${ctx}/sms/jmsgGatewaySign/form">信息添加</a></li> --%>
			<li><a href="${ctx}/sms/smsSign/config">信息添加</a></li>
		</shiro:hasPermission>
	</ul>
	<form:form id="searchForm" modelAttribute="smsSign" action="${ctx}/sms/smsSign/" method="post" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="100"/>
		<ul class="ul-form">
			<li><label>通道：</label>
				<form:select path="gwCode" class="input-large">
...
<!--items，自定义标签待实现  -->	  
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
		<c:forEach items="${page.list}" var="smsSign">
			<tr onclick="selectTr(this, '${smsSign.id}' ,'id')">
				<td>
					<input type="checkbox" name="id" id="${smsSign.id}" value="${smsSign.id}_${smsSign.user.id}_${smsSign.gwCode}_${smsSign.sign}" 
					onclick="if(this.checked){this.checked=false;}else{this.checked=true;}">
				</td>
				<shiro:hasPermission name="sms:smsSign:edit"><td>
<!--测试功能待实现  -->				<!-- <a href="javascript:showTestModal('')">测试
					       </a> | --> 
					<a href="${ctx}/sms/smsSign/form?id=${smsSign.id}">修改</a> |
					...
					<a href="#" 
					onclick='sigleDelete("${smsSign.id}","${smsSign.user.id}","${smsSign.gwCode}","${smsSign.sign}")'>删除</a>
				...
				<td>
					<fmt:formatDate value="${smsSign.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/>
				</td>
				<td>
					${smsSign.remarks}
				</td>
