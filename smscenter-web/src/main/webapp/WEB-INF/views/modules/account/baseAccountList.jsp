...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/account/baseAccount/">信息列表</a></li>
		<shiro:hasPermission name="account:baseAccount:edit"><li><a href="${ctx}/account/baseAccount/form">信息添加</a></li></shiro:hasPermission>
	</ul>
	<form:form id="searchForm" modelAttribute="baseAccount" action="${ctx}/account/baseAccount/" method="post" class="breadcrumb form-search">
		...
		<c:forEach items="${page.list}" var="baseAccount">
			<tr>
				...
				<shiro:hasPermission name="account:baseAccount:edit"><td>
    				...
    				<a href="${ctx}/account/baseAccount/form?id=${baseAccount.id}">修改</a>
    				<a href="javascript:windowOpen('${debug}/cache.jsp?type=4&key=${baseAccount.userid}','用户账号信息',900,550);">调试</a>
    				<c:if test="${baseAccount.accStatus eq 1 || baseAccount.accStatus eq -1}">
					<a href="${ctx}/account/baseAccount/updateAccStatus?id=${baseAccount.id}&oldAccStatus=${baseAccount.accStatus}&userid=${baseAccount.userid}" onclick="return confirmx('确认要${baseAccount.accStatus eq 1 ? '下线':'上线' }该账户吗？', this.href)">${baseAccount.accStatus eq 1 ? '下线':'上线' }</a>
					</c:if>
	                     		...
	                     		<div class="control-group">
									...
									<div class="controls">
										<select id="userid" name="id" class="input-xlarge required">
											<c:forEach items="${fns:getUserList('')}" var="item">
