...
public class JedisUtils {
	...
	public static String get(String key) {
		...
		try {
			...
			if (jedis.exists(key)) {
				value = jedis.get(key);
				value = StringHelper.isNotBlank(value) && !"nil".equalsIgnoreCase(value) ? value : null;
		...
		try {
			...
			if (jedis.exists(getBytesKey(key))) {
				...
				for (Map.Entry<byte[], byte[]> e : map.entrySet()){
					value.put(StringHelper.toString(e.getKey()), toObject(e.getValue()));
				}
...
//			logger.debug("getResource.", jedis);
	...
	public static Object getObjectKey(byte[] key){
		try{
			return StringHelper.toString(key);
