...
@RestController
@RequestMapping(value = "/api/sms/upstream")
public class SmsGatewayUpstream {

    private final static Logger logger = Logger.getLogger(SmsGatewayUpstream.class);
    ...
    public String push(String code, String respText, HttpServletRequest request, HttpServletResponse response){
        ...
        SmsHttpMsgResponse httpMsgResponse = new SmsHttpMsgResponse("");
        ...
        if (StringUtils.isNotBlank(respText)) {
            ...
            if( executer != null) 
            {
            	httpMsgResponse = executer.deliver(respText, false);
            }
        }else{
        ...
        try {
            response.reset();
            response.setContentType(httpMsgResponse.getContentType());
            response.setCharacterEncoding(httpMsgResponse.getCharacterEncoding());
            response.getWriter().print(httpMsgResponse.getResult());
            return null;
        } catch (IOException e) {
            return null;
        }
